...
## 3. Αλλαγή του κωδικού διαχείρισης

Ο προεπιλεγμένος κωδικός είναι **`changeme`**. Άλλαξέ τον πριν βάλεις
πραγματικά δεδομένα:

1. Στο τερματικό (τοπικά ή στον server), τρέξε:
   ```
   php -r "echo password_hash('ο-δικος-μου-κωδικος', PASSWORD_DEFAULT), PHP_EOL;"
   ```
2. Δημιούργησε το αρχείο `config.local.php` (δεν ανεβαίνει στο git) με περιεχόμενο:
   ```php
   <?php
   define('APP_PASSWORD_HASH', '$2y$10$...το-hash-που-πήρες...');
   ```
...
```
config.php              ρυθμίσεις (κωδικός, timezone, path βάσης)
config.local.php        τοπικός κωδικός (προαιρετικό, στο .gitignore)
...
includes/auth.php       login gate + CSRF
...
- Ο κωδικός αποθηκεύεται μόνο ως bcrypt hash, ποτέ σε καθαρό κείμενο.
